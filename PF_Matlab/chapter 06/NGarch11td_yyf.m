function [ cvars,zs, rets, params,likelihood_val ] = NGarch11_yyftd(Prices,dd,vals)
% The function implements NGarch11 model
% Warning: we accept prices as inputs instead of log-returns
% Inputs: (Prices, dd, vals)
% Prices: stock prices
% dd : dates
% vals : initial paramters
% Outputs: [ cvars,zs, rets, params,likelihood_val ] 
% cvars             :conditional variances,or the  
%                         square of volatilities or the square of sigma
% zs                  :standardirized returns
% rets                :log-returns 
% params          :estimated paramters
% likelihood_val :likelihood value
...
phi0	=	[alpha,beta,omega,theta,d];
options	=	optimset('MaxFunEvals',100000,'Maxiter',100000,'Display','iter','LargeScale','off');
[phi,likelihood_val]=fminsearch(@(phi)likfunc_levrgefftd(phi,rets),vals);
...
if 1==1
    ...
    display(['|initial dd------------------|',num2str(vals(5)),'|'])
	display('|----------------------------------------------------|')
    ...
    display(['|estimated dd------------------|',num2str(params(5)),'|'])
